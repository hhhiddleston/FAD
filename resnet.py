...
from utils import GumbelAcc, Samplewise_Weighted_CrossEntropyLoss, GumbelTPR, GumbelTNR
...
class ResNet(nn.Module):
    def __init__(
        ...
        self.fc_in_dim = 512 * block.expansion
...
def resnet18(num_classes: int, pretrained: bool = False, freeze_pretrain: bool = False, progress: bool = True, **kwargs: Any) -> ResNet:
    ...
    """
    _num_classes = 1000 if pretrained else num_classes
    model = _resnet("resnet18", _num_classes, BasicBlock, [2, 2, 2, 2], pretrained, progress, **kwargs)
    if pretrained:
        if freeze_pretrain:
            for p in model.parameters():
                p.requires_grad = False
        del model.fc
        model.fc = nn.Linear(model.fc_in_dim, num_classes)
    return model

def resnet34(num_classes: int, pretrained: bool = False, freeze_pretrain: bool = False, progress: bool = True, **kwargs: Any) -> ResNet:
    ...
    """
    _num_classes = 1000 if pretrained else num_classes
    model =  _resnet("resnet34", _num_classes, BasicBlock, [3, 4, 6, 3], pretrained, progress, **kwargs)
    if pretrained:
        if freeze_pretrain:
            for p in model.parameters():
                p.requires_grad = False
...
def resnet50(num_classes: int, pretrained: bool = False, freeze_pretrain: bool = False, progress: bool = True, **kwargs: Any) -> ResNet:
    ...
    """
    _num_classes = 1000 if pretrained else num_classes
    model =  _resnet("resnet50", _num_classes, Bottleneck, [3, 4, 6, 3], pretrained, progress, **kwargs)
    if pretrained:
        if freeze_pretrain:
            for p in model.parameters():
                p.requires_grad = False
